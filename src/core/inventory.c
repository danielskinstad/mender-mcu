...
mender_err_t
mender_inventory_init(uint32_t interval, const char *dev_type, uint32_t backoff_interval, uint32_t max_backoff_interval) {
    mender_err_t ret;
    ...
    inventory_work_params.backoff.interval     = (0 != backoff_interval ? backoff_interval : CONFIG_MENDER_NETWORK_ERROR_BACKOFF);
    inventory_work_params.backoff.max_interval = (0 != max_backoff_interval ? max_backoff_interval : CONFIG_MENDER_NETWORK_ERROR_MAX_BACKOFF);
